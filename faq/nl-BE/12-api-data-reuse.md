...
Dat kan afhangen van het land, en of het netwerk traag of duur is. Alles boven de 5000 pixels in breedte of hoogte is waarschijnlijk niet erg nuttig. En als je op de een of andere manier kunt vaststellen dat het netwerk traag is, dan zou zelfs een afbeelding van 2000 pixels al geweldig zijn (zeker beter dan helemaal geen afbeelding!).
...
## Kan ik via de API zoeken op een specifieke productnaam?

Helaas is het via de API nog niet mogelijk om eenvoudig en specifiek op productnaam te zoeken.

Door een filter op categorie te gebruiken, kunt u uw zoekopdracht wellicht nauwkeuriger maken.
...
U kunt ook het volgende raadplegen:

- Onze API-documentatie: [ https://openfoodfacts.github.io/api-documentation/](https://openfoodfacts.github.io/api-documentation/)
- Gebruiksvoorwaarden van Open Food Facts: [ ](https://world.openfoodfacts.org/terms-of-use)[https://world.openfoodfacts.org/terms-of-use](https://world.openfoodfacts.org/terms-of-use)
- Op basis van onze gegevens: [ https://world.openfoodfacts.org/data](https://world.openfoodfacts.org/data)

---

## Zijn er voorwaarden verbonden aan het gebruik van de API?

Alle documentatie over het gebruik van de API is te vinden op de [API-documentatiepagina](https://openfoodfacts.github.io/openfoodfacts-server/api/), maar hier is een korte samenvatting:

- De Open Food Facts-database is beschikbaar als open data onder de Open Database License (ODbL). Zie [https://world.openfoodfacts.org/terms-of-use](https://world.openfoodfacts.org/terms-of-use) voor de juridische details. De twee voorwaarden zijn toeschrijving en gelijk delen. Als je gegevens uit Open Food Facts combineert met andere databases, vereist de ODbL dat de resulterende database ook als open data wordt vrijgegeven. Dit betekent ook dat je de gegevens alleen kunt combineren met bronnen die een dergelijke herverdeling toestaan.

- Je moet **altijd** een aangepaste User-Agent gebruiken bij het uitvoeren van API-aanroepen om je app te identificeren.

- Voor elk API-eindpunt gelden snelheidslimieten.
